...
- GPS 集約（移動中は経度・緯度を 3 次スプライン / 停止時は経度・緯度を平均、標高は直近有効値）が実移動で自然か実機確認する
...
- `TYPE_ROTATION_VECTOR` 優先の世界座標変換で `KAccelSource=WORLD_ROTATION_VECTOR` が補助CSV/Roomに保存されるか、また右左折・カーブ時に `trK` の水平最大加速度しきい値で GPS 即時取得が増えるか実機確認する
...
- `STAY` 領域が app 地図・地図ウィジェットの折れ線で保存済み stay point 1 点に畳まれて表示されるか実機確認する
