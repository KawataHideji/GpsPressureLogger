...
- 標準バックアップは `timestamp` 昇順のページングで読み出し、全レコードを一度にメモリへ載せず、ZIP 内 CSV へ逐次書き込む。
...
- インポートは一定件数ごとに書き込み、既存 `Timestamp` の確認も複数件を一括照会して、レコードごとの個別 SQL を行わない。
...
- グラフのピンチインは設定表示期間の最大 6 倍まで表示でき、その範囲の履歴は必要になった時点で読み込む。
- グラフは単一のポインタ入力処理で、1 本指を時間移動、2 本指以上をピンチ中心時刻を保持した倍率変更として扱う。
...
- グラフの操作中は現在の `ProcessedSeries` を再利用し、DB 再読込と系列再集計は指を離して表示範囲が確定した後にだけ行う。
...
- Android アプリ本体の地図画面は、過去日は対象日を開いた時点で DB から一度だけ読み込む。現在の論理日は 30 秒間隔で更新し、3 秒ごとのログ追加では全日分を再処理しない。
- 地図の停止補正、軌跡補間、停止クラスタ、モード別線分、GPS 欠損線分の生成はバックグラウンドで行い、UI スレッドでは生成済み結果から overlay を登録する。
